...
if __name__ == "__main__":
    ...
    dcgan.load_weights(generator_file="generator ({})4000.h5".format(
        dcgan.name), discriminator_file="discriminator ({}).h5".format(dcgan.name))
    ...
    while (time <= maxTime):
        ...
        for i in range(len(seed_start)):
            ...
            for j in range(dcgan.latent_dim):
                vary[i][j] = seed_start[i][j] + \
                    np.sin(2*np.pi*(time/maxTime) * latentSpeed[i][j])

        gen_imgs = dcgan.generator.predict(vary)

        create_image(
            gen_imgs, "images/goodsell_{0:05d}.png".format(frameCount))
